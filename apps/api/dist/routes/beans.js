...
export async function beansRoutes(server) {
    // GET /api/beans
    ...
    server.get('/beans/:id', async (request, reply) => {
        ...
        const actualNotesCount = {};
        for (const brew of allBrews) {
            if (brew.tastingNotesActual) {
                const notes = JSON.parse(brew.tastingNotesActual);
    ...
    server.patch('/beans/:id/recipes/:methodId', async (request, reply) => {
        const { id: beanId, methodId } = request.params;
        const { grinderTarget, recipeOverrides } = request.body;
