...
export const METHOD_ORDER = ['v60', 'moka', 'espresso', 'french_press'] as const;

export type MethodName = keyof typeof DEFAULT_METHODS;

// V60 scaling: scale all pours proportionally
export function scaleV60Recipe(
  baseRecipe: {
    dose: number;
    ratio: number;
    steps: Array<{ waterRatio?: number; [key: string]: unknown }>;
  },
  scaleFactor: number
): {
  dose: number;
  water: number;
  steps: Array<{ waterAmount?: number; [key: string]: unknown }>;
} {
...
export function scaleMokaRecipe(
  baseRecipe: {
    dose: number;
